  ...
  if (res == H_OK) {
    if (json_array_size(j_result)) {
      ...
      if (res == H_OK) {
        ...
        y_log_message(Y_LOG_LEVEL_ERROR, "get_client - Error executing j_query for client %s", json_string_value(json_object_get(json_array_get(j_result, 0), "name")));
      }
    ...
    y_log_message(Y_LOG_LEVEL_ERROR, "get_client - Error executing j_query");
    j_return = json_pack("{si}", "result", E_ERROR_DB);
  }
  return j_return;
}
  ...
  if (res == H_OK) {
    if (json_array_size(j_result)) {
      ...
      if (res == H_OK) {
        ...
        y_log_message(Y_LOG_LEVEL_ERROR, "get_client_from_id - Error executing j_query for client %s", json_string_value(json_object_get(json_array_get(j_result, 0), "name")));
      }
      j_return = json_pack("{sisO}", "result", E_OK, "client", json_array_get(j_result, 0));
    } else {
      j_return = json_pack("{si}", "result", E_ERROR_NOT_FOUND);
    ...
    y_log_message(Y_LOG_LEVEL_ERROR, "get_client_from_id - Error executing j_query");
    j_return = json_pack("{si}", "result", E_ERROR_DB);
  }
  return j_return;
}
...
int set_client(struct config_elements * config, json_t * j_client, json_int_t ec_id) {
  ...
  j_query = json_pack("{sss{sOss}s{sI}}",
                      ...
                      "where",
                        "ec_id", ec_id);
  if (json_object_get(j_client, "client_secret") != NULL) {
    json_object_set(json_object_get(j_query, "set"), "ec_client_secret", json_object_get(j_client, "client_secret"));
  }
...
json_t * update_client_registration(struct config_elements * config, json_t * j_client_database, json_t * j_registration) {
  ...
  if (i_init_session(&i_session) == I_OK) {
    if (i_set_parameter_list(&i_session, I_OPT_ACCESS_TOKEN, json_string_value(json_object_get(j_client_database, "registration_access_token")),
      ...
      json_object_del(j_registration, "registration_access_token");
      json_object_del(j_registration, "registration_client_uri");
