...
int verify_access_token(struct config_elements * config, struct _i_session * i_session) {
  ...
  if (config->oidc_is_jwt_access_token) {
    if (i_verify_jwt_access_token(i_session, NULL) == I_OK) {
      j_introspection = json_incref(i_session->access_token_payload);
    }
  } else {
    if (i_set_str_parameter(i_session, I_OPT_TOKEN_TARGET, i_get_str_parameter(i_session, I_OPT_ACCESS_TOKEN)) == I_OK) {
      if (i_get_token_introspection(i_session, &j_introspection, I_INTROSPECT_REVOKE_AUTH_CLIENT_TARGET, 0) != I_OK) {
            ...
            if (now > json_integer_value(json_object_get(json_array_get(j_result, 0), "expires_at"))) {
              if (json_string_length(json_object_get(json_array_get(j_result, 0), "refresh_token"))) {
                if (i_init_session(&i_session) == I_OK) {
                  if (i_session_setup_connection(config, &i_session) == E_OK) {
                    if (i_set_parameter_list(&i_session, I_OPT_REFRESH_TOKEN, json_string_value(json_object_get(json_array_get(j_result, 0), "refresh_token")),
                                                         I_OPT_RESPONSE_TYPE, I_RESPONSE_TYPE_REFRESH_TOKEN,
                                                         I_OPT_NONE) == I_OK) {
                      if (i_run_token_request(&i_session) == I_OK) {
                        if ((res = verify_access_token(config, &i_session)) == E_OK) {
                          if (i_get_int_parameter(&i_session, I_OPT_EXPIRES_IN)) {
                            token_expire = now + (time_t)i_get_int_parameter(&i_session, I_OPT_EXPIRES_IN);
                          } else {
                          ...
                          j_query = json_pack("{sss{s{ss}ss*}s{sO}}",
                                              ...
                                              "set",
                                                "s_token_expires_at",
                                                  "raw", expire_clause,
                                                "s_refresh_token", i_get_str_parameter(&i_session, I_OPT_REFRESH_TOKEN),
                                              "where",
                          ...
                          json_decref(j_query);
                          if (res == H_OK) {
                            if (i_get_str_parameter(&i_session, I_OPT_REVOCATION_ENDPOINT) != NULL) {
                              if (i_set_str_parameter(&i_session, I_OPT_TOKEN_TARGET, i_get_str_parameter(&i_session, I_OPT_ACCESS_TOKEN)) == I_OK) {
                    ...
                    y_log_message(Y_LOG_LEVEL_ERROR, "check_session - Error i_session_setup_connection");
                    j_return = json_pack("{si}", "result", E_ERROR);
                  }
                  i_clean_session(&i_session);
                } else {
                  y_log_message(Y_LOG_LEVEL_ERROR, "check_session - Error i_init_session");
                  j_return = json_pack("{si}", "result", E_ERROR);
                }
              } else {
...
json_t * init_session(struct config_elements * config, const char * cur_session_id, int create) {
  ...
  if (i_init_session(&i_session) == I_OK) {
    if (i_session_setup_connection(config, &i_session) == E_OK) {
          ...
          if (create) {
            if (rand_string(session_id, ESRAS_SESSION_LENGTH) != NULL && generate_hash(session_id, session_hash)) {
              ...
              j_query = json_pack("{sss{sssssss{ss}}}",
                                  ...
                                  "values",
                                    "s_session_hash", session_hash,
                                    "s_state", i_get_str_parameter(&i_session, I_OPT_STATE),
                                    "s_nonce", i_get_str_parameter(&i_session, I_OPT_NONCE),
                                    "s_expires_at",
              ...
              json_decref(j_query);
              if (res == H_OK) {
                j_return = json_pack("{sis{ssss*}}", "result", E_OK, "session", "session_id", session_id, "auth_url", i_get_str_parameter(&i_session, I_OPT_REDIRECT_TO));
              } else {
            ...
            if (generate_hash(cur_session_id, session_hash)) {
              ...
              json_decref(j_query);
              if (res == H_OK) {
                j_return = json_pack("{sis{ssss*}}", "result", E_OK, "session", "session_id", cur_session_id, "auth_url", i_get_str_parameter(&i_session, I_OPT_REDIRECT_TO));
              } else {
                y_log_message(Y_LOG_LEVEL_ERROR, "init_session - Error executing j_query");
                j_return = json_pack("{si}", "result", E_ERROR_DB);
              }
      ...
      y_log_message(Y_LOG_LEVEL_ERROR, "init_session - Error i_session_setup_connection");
      j_return = json_pack("{si}", "result", E_ERROR);
    }
    i_clean_session(&i_session);
  } else {
    y_log_message(Y_LOG_LEVEL_ERROR, "init_session - Error i_init_session");
    j_return = json_pack("{si}", "result", E_ERROR);
  }
  return j_return;
}
...
int validate_session_code(struct config_elements * config, const char * session_id, const char * state, const char * code) {
  ...
  if (i_init_session(&i_session) == I_OK) {
    if (i_session_setup_connection(config, &i_session) == E_OK) {
          ...
          if (json_array_size(j_result)) {
            if (i_set_parameter_list(&i_session, I_OPT_RESPONSE_TYPE, I_RESPONSE_TYPE_CODE,
              ...
              if ((res = i_run_token_request(&i_session)) == I_OK &&
                  json_string_length(json_object_get(i_session.id_token_payload, "sub"))) {
                if ((res = verify_access_token(config, &i_session)) == E_OK) {
                  j_query = json_pack("{sss[ss]s{ss}}",
                                      ...
                                      "columns",
                                        ...
                                        "p_name AS name",
                                      "where",
                                        "p_sub", json_string_value(json_object_get(i_session.id_token_payload, "sub")));
                  res = h_select(config->conn, j_query, &j_result_profile, NULL);
                  json_decref(j_query);
                  if (res == H_OK) {
                    if (json_array_size(j_result_profile)) {
                      time(&now);
                      if (i_get_int_parameter(&i_session, I_OPT_EXPIRES_IN)) {
                        now += (time_t)i_get_int_parameter(&i_session, I_OPT_EXPIRES_IN);
                      } else {
                      ...
                      json_decref(j_query);
                      if (res == H_OK) {
                        if (json_string_length(json_object_get(i_session.id_token_payload, config->oidc_name_claim))) {
                          j_query = json_pack("{sss{ss}s{sO}}",
                                              "table", ESRAS_TABLE_PROFILE,
                                              "set",
                                                "p_name", json_string_value(json_object_get(i_session.id_token_payload, config->oidc_name_claim)),
                                              "where",
                                                "p_id", json_object_get(json_array_get(j_result_profile, 0), "p_id"));
                          res = h_update(config->conn, j_query, NULL);
                          json_decref(j_query);
      ...
      y_log_message(Y_LOG_LEVEL_ERROR, "validate_session_code - Error i_session_setup_connection");
      ret = E_ERROR;
    }
    i_clean_session(&i_session);
  } else {
    y_log_message(Y_LOG_LEVEL_ERROR, "validate_session_code - Error i_init_session");
    ret = E_ERROR;
  }
    ...
    if (i_session_setup_connection(config, &i_session) == E_OK) {
      if (generate_hash(session_id, session_hash)) {
        if (i_get_str_parameter(&i_session, I_OPT_REVOCATION_ENDPOINT) != NULL) {
          ...
          if (res == H_OK) {
            if (i_get_str_parameter(&i_session, I_OPT_REVOCATION_ENDPOINT) != NULL) {
              if (i_set_str_parameter(&i_session, I_OPT_TOKEN_TARGET, json_string_value(json_object_get(json_array_get(j_result, 0), "refresh_token"))) == I_OK) {
                if (i_revoke_token(&i_session, I_INTROSPECT_REVOKE_AUTH_CLIENT_TARGET) != I_OK) {
      ...
      y_log_message(Y_LOG_LEVEL_ERROR, "delete_session - Error i_session_setup_connection");
      ret = E_ERROR;
    }
    i_clean_session(&i_session);
  } else {
    y_log_message(Y_LOG_LEVEL_ERROR, "delete_session - Error i_init_session");
    ret = E_ERROR;
  }
  return ret;
}
