      ...
      <div>
        ...
        <form onSubmit={(e) => this.verifyRegistration(e)}>
          ...
          <div className="form-group">
            ...
            <textarea className="form-control" id="client_jwks" placeholder="{  keys: [  {  kty:[...]" value={this.state.client.jwks} onChange={(e) => this.changeText(e, 'jwks')}/>
            <small id="jwksHelp" className="form-text text-muted">{i18next.t("register_client_jwks_help")}</small>
          </div>
          <hr/>
          <div className="form-group">
            <button type="submit" className="btn btn-primary elt-left">{i18next.t("submit")}</button>
            <button type="button" className="btn btn-primary">{i18next.t("cancel")}</button>
          </div>
        </form>
      </div>
    );
